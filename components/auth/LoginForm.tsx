...
import { signInWithEmail, signInWithGoogle, signInWithFacebook } from '@/app/login/actions'
  ...
  return (
    <div className="w-full">
      ...
      <div className="space-y-2">
        <OAuthButton onClick={handleGoogleLogin} disabled={isPending}>
          <GoogleIcon />
          Continue with Google
        </OAuthButton>

        <OAuthButton onClick={handleFacebookLogin} disabled={isPending}>
          <FacebookIcon />
      ...
      <p className="mt-5 text-center text-xs" style={{ color: 'var(--text-muted)' }}>
        ...
        <Link href="/signup" className="underline-offset-2 hover:underline" style={{ color: 'var(--text-secondary)' }}>
          Sign up free
        </Link>
      </p>
    </div>
